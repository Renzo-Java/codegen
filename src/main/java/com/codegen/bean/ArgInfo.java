...
import com.codegen.util.TemlateEnum;
...
//    private String supportCodeName = "";//支持编码转换-描述值字段
//    private String supportCodePy = ""; //支持编码转换-拼音值字段,可以为空
//    
//    private String supportFlagName = "";//支持状态字段
        ...
        for (TemlateEnum tmpEnum : TemlateEnum.values()) {
            if (tmpNo == tmpEnum.getTmpNo()) {
                if (tmpEnum.getIsJavaFile()) {
                    if (tmpNo == 2 || tmpNo == 12) {//测试类的
                        filePath = outputFile.getSrcTestPath();
                        filepath = filePath + pkgPath + "/" + className;
                    }
                    else {
                        ...
                        filepath = filePath + pkgPath + "/" + tmpEnum.getPkgSux().replace(".", "/") + "/" + className;
                    }
                    filepath = filepath + tmpEnum.getSuffix();
                }
                else {
                    filePath = outputFile.getSrcConfPath();
                    String pre = tmpEnum.getPkgSux() + "/";
                    String lastPkg = outputFile.getLastPkg();
                    if (lastPkg != null && lastPkg.length() > 0)
                        pre += lastPkg + "/";
                    ...
                    if (filepath == null) {
                        filepath = filePath + pre + className;//className前面可以加上包名： + pkg + "." 
                        filepath = filepath + tmpEnum.getSuffix();
                    }
                }
...
//    public String getSupportFlagName() {
//		return supportFlagName;
//	}
//
//	public void setSupportFlagName(String supportFlagName) {
//		this.supportFlagName = supportFlagName;
//	}
...
//	public String getSupportCodeName() {
//		return supportCodeName;
//	}
//
//	public void setSupportCodeName(String supportCodeName) {
//		this.supportCodeName = supportCodeName;
//	}
//
//	public String getSupportCodePy() {
//		return supportCodePy;
//	}
//
//	public void setSupportCodePy(String supportCodePy) {
//		this.supportCodePy = supportCodePy;
//	}
