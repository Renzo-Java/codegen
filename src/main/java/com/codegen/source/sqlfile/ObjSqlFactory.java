 ...
 * 修改内容: 
 */
package com.codegen.source.sqlfile;
...
import com.codegen.source.DefaultObjFactory;
...
public class ObjSqlFactory implements ObjFactory {
    ...
    private List<Column> parseSql(String sql) {
        ...
        do {
            ...
            if(temps.length > 1) {
                ...
                if (colType.equalsIgnoreCase("BIGINT") && length <= 0)
     ...
     * @return
     * @author admin created on 2014-4-25
     */
    private String parserComment(String[] temps) {
        for (int ii = 0; ii < temps.length; ii++) {
        	String tmpStr = temps[ii];
            if ("COMMENT".equalsIgnoreCase(tmpStr)) {
                String comment = "";
                ...
                return comment;                
            } else if (tmpStr != null && tmpStr.toUpperCase().startsWith("COMMENT")) {
            	return StringUtils.substringBeforeLast(StringUtils.substringAfter(tmpStr, "'"), "'");
            }
        }
        return null;
    }
